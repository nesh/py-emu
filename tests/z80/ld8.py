...
def _check_ld_r_r(src, dst):
    ...
    setattr(cpu, sreg, 0x10)
    if sreg != dreg:
        setattr(cpu, dreg, 0x8A)
    cpu.write(0x0000, op)
    cpu.run()
    
    assert getattr(cpu, dreg) == 0x10
...
def test_ld_r_r():
    for dst in range(0, 0x08):
        for src in range(0, 0x08):
            ...
            sreg = REGS_SRC[src]
            dreg = REGS_SRC[dst]
            _check_ld_r_r.description = 'Z80: LD %s,%s' % (sreg, dreg)
            yield _check_ld_r_r, src, dst

def _check_ld_r_n(regb):
    op = (regb << 3) + 0x06
    reg = REGS_SRC[regb]
    ...
    cpu.F.byte = 0x00
    cpu.write(0x0000, op)
    cpu.write(0x0001, 0xA5)
    cpu.run()
    
    assert getattr(cpu, reg) == 0xA5
    assert cpu.F.byte == 0x00
...
def test_ld_r_n():
    for b in range(0, 0x08):
        try:
            ...
            continue
        _check_ld_r_n.description = 'Z80: LD %s,A5h' % dreg
        yield _check_ld_r_n, b
