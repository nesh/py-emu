...
class MOS6502(CPU, LowMOS6502MemMixin, LowMOS6502BxxMixin):
    ...
    def push(self, val):
        self.mem.write(0x100 + self.SP, val)
        self.SP = (self.SP - 1) & 0xFF
    
    def push_word(self, val):
        self.mem.write(0x100 + self.SP, val >> 8)
        self.SP = (self.SP - 1) & 0xFF
        
        self.mem.write(0x100 + self.SP, val & 0x00FF)
        self.SP = (self.SP - 1) & 0xFF
    
    def pop(self):
        self.mem.read(0x100 + self.SP)
        self.SP = (self.SP + 1) & 0xFF
        return self.mem.read(0x100 + self.SP)
    
    def pop_word(self):
        lo = self.mem.read(0x100 + self.SP)
        self.SP = (self.SP + 1) & 0xFF
        hi = self.mem.read(0x100 + self.SP)
        self.SP = (self.SP + 1) & 0xFF
        return lo | (hi << 8)
    ...
    def brk(self, adr, op):
        ...
        self.P.i = True
        self.PC = self.read_word(MOS6503.BRK_VECTOR)
    ...
    def las(self, adr, op):
        ...
        self.A = self.X = self.SP = self.SP & op
        self.P.mset(**SZ_TAB[self.A])
