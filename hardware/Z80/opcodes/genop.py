...
def convert_to_table(out, type_, table):
    ...
    print >>out, '%s%s = [InvalidOp] * 0x100' % (IDENT*2, tabname)
...
def convert_to_py(out, type_, code, op, tstates):
    ...
    # prefixes are handled differently
    if opcode in ('SHIFT', 'IGNORE', 'RESET',): return False
    
    if opcode not in CMDS:
        warnings.warn('uninplemented %s (%X)' % (opcode, code))
        return False
...
def process_ofile(fname, name, dasm_fh):
    ...
    print >>fout, 'from hardware.cpu import CPUTrapInvalidOP, InvalidOp\n'
