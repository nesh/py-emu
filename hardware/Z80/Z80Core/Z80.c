...
#define WrZ80(Addr, Value) (*R->write)((Addr), (Value))
#define RdZ80(Addr) (*R->read)((Addr))
#define OpZ80(Addr) (*R->read_op)((Addr))
...
/************************************ TO BE WRITTEN BY USER **/
#define OutZ80(Port, Value) (*R->io_write)((Port), (Value))
#define InZ80(Port) (*R->io_read)((Port))
...
/************************************ TO BE WRITTEN BY USER **/
#define PatchZ80(R) if(*R->patch) (*R->patch)(R)
...
/************************************ TO BE WRITTEN BY USER **/
#define JumpZ80(PC) if(*R->jump) (*R->jump)(PC)

// reg SHORTCUTS
#define F R->AF.B.l
#define A R->AF.B.h

#define CYCLES_ADD(c)   R->ICount -= c; R->ITotal += c
#define CYCLES_SUB(c)   R->ICount += c; R->ITotal -= c
#define SET_R(val) R->R = (val) // & 0x7F
#define INC_R SET_R(R->R + 1)

#define S(Fl) F |= Fl
#define R(Fl) F &= ~(Fl)
#define FLAGS(Rg,Fl) F = Fl | ZSTable[Rg]

#define FLAGS_XY(val) ((val) & (Y_FLAG | X_FLAG))
    
#define MAKE_XY_IN(val) \
    if(val & X_FLAG) S(X_FLAG); else R(X_FLAG); \
    if(val & Y_FLAG) S(Y_FLAG); else R(Y_FLAG);

#define M_RLC(Rg) \
    F = Rg >> 7; \
    Rg = (Rg << 1) | F; \
    F |= PZSTable[Rg] | FLAGS_XY(Rg)
...
#define M_RL(Rg) \
    if(Rg&0x80) { \
        Rg=(Rg << 1) | (F & C_FLAG); \
        F = PZSTable[Rg] | C_FLAG | FLAGS_XY(Rg); \
    } else { \
        Rg = (Rg << 1) | (F & C_FLAG); \
        F = PZSTable[Rg] | FLAGS_XY(Rg); \
    }
...
#define M_BIT(Bit,Rg) \
    F = (F & C_FLAG) | H_FLAG | PZSTable[Rg & (1 << Bit)] \
    | (((Bit == 5) && (Rg & (1 << Bit))) ? Y_FLAG : 0) \
    | (((Bit == 3) && (Rg & (1 << Bit))) ? X_FLAG : 0)

#define M_SET(Bit,Rg) Rg |= 1 << Bit
#define M_RES(Bit,Rg) Rg &= ~(1 << Bit)
#define M_POP(Rg) \
  R->Rg.B.l = OpZ80(R->SP.W++); \
  R->Rg.B.h = OpZ80(R->SP.W++)
#define M_PUSH(Rg) \
  WrZ80(--R->SP.W,R->Rg.B.h); \
  WrZ80(--R->SP.W,R->Rg.B.l)
#define M_CALL \
  J.B.l = OpZ80(R->PC.W++); \
  J.B.h = OpZ80(R->PC.W++); \
  WrZ80(--R->SP.W,R->PC.B.h); \
  WrZ80(--R->SP.W,R->PC.B.l); \
  R->PC.W = J.W; \
  JumpZ80(J.W)
#define M_JP \
  J.B.l = OpZ80(R->PC.W++); \
...
#define M_JR \
  ...
  JumpZ80(R->PC.W)
#define M_RET \
  R->PC.B.l = OpZ80(R->SP.W++); \
  R->PC.B.h = OpZ80(R->SP.W++); \
  JumpZ80(R->PC.W)
#define M_RST(Ad) \
  WrZ80(--R->SP.W, R->PC.B.h); \
  WrZ80(--R->SP.W,R->PC.B.l); \
...
#define M_AND(Rg) \
    A &= Rg; \
    F = H_FLAG | PZSTable[A] | FLAGS_XY(A)
    
#define M_OR(Rg) \
        A |= Rg; \
        F = PZSTable[A] | FLAGS_XY(A)
...
#define M_IN(Rg) \
  Rg = InZ80(R->BC.W); \
  F = PZSTable[Rg] | (F & C_FLAG) | FLAGS_XY(Rg)
...
INLINE int CodesCB(register Z80 *R) {
    register byte I;

    I = OpZ80(R->PC.W++);
    CYCLES_ADD(CyclesCB[I]);

    switch (I) {
        #include "CodesCB.h"
        default:
            R->Trap = R->PC.W - 2;
            return EXECZ80_UNKNOWN_OP;
            break;
    }
    return EXECZ80_OK;
}

INLINE int CodesDDCB(register Z80 *R) {
    register pair J;
    register byte I;
...
INLINE int CodesFDCB(register Z80 *R) {
    register pair J;
    register byte I;
...
INLINE int CodesED(register Z80 *R) {
    register byte I;
    register pair J;
    register byte n;

    I = OpZ80(R->PC.W++);
    CYCLES_ADD(CyclesED[I]);
...
INLINE int CodesDD(register Z80 *R) {
    register byte I;
    register pair J;

    #define XX IX
    I = OpZ80(R->PC.W++);
    CYCLES_ADD(CyclesXX[I]);
...
INLINE int CodesFD(register Z80 *R) {
    register byte I;
    register pair J;
    ...
    for (R->ICount = RunCycles;;) {
        while (R->ICount > 0) {
            ...
            switch (I) {
                ...
                default:
                    R->Trap = R->PC.W - 1;
                    ret = EXECZ80_UNKNOWN_OP;
                    break;
            }
        }

        if((R->TrapBadOps) && (ret != EXECZ80_OK)) return ret;
